...
**MUST**: All implementations MUST validate incoming data against the schemas defined in this document.  
**MUST**: All implementations MUST produce data conforming to these schemas.  
**SHOULD**: Implementations SHOULD use JSON Schema for validation where applicable.

## Task Schema

A Task is the atomic unit of execution within a Flow. It represents a single unit of work that can be executed by an Executor.
...
**SHOULD**:
- `name` SHOULD match a registered executor identifier when `schemas.method` is present.
- `inputs` SHOULD conform to `schemas.input_schema` if `schemas.input_schema` is present.
- `created_at`, `updated_at`, `started_at`, `completed_at` SHOULD use UTC timezone.
